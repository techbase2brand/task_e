...
const AddModule: React.FC<any> = ({ navigation, classes }) => {
  ...
  const validateModule = () => {
    ...
    module.modules.forEach((value, index) => {
      if (value.trim() === "") {
        valid = false;
      }
      const otherModules = [...module.modules];
      otherModules.splice(index, 1);
      if (otherModules.includes(value)) {
    ...
    if (!valid) {
      ...
      alert(
        `Duplicate modules (${duplicates.join(", ")}) are not allowed for the same project and phase combination.`
      );
      valid = false;
    }
    return valid;
